...
- ✅ **Rediseño splash LIGIE — EN VIVO en bondiact.io/ligie**: aplicado a `ligie/index.html` (solo CSS del `#splash`). Fondo azul profundo diagonal (claro arriba-izq → oscuro abajo-der) + foto de puerto (`images/ligie-bg-puerto.jpg`, ruta `../images/`) + malla + ícono torre de control + botón/barra en degradado. Texto del Aviso, las **11,494 fracciones**, el buscador y el login de Google quedaron **INTACTOS**. Respaldo: `ligie/index.html.bak-pre-splash-*` (gitignored).
- 📋 **Sprint cerrado en Monday** → grupo **"Sprint 43 — Done ✅"** (S43-01/02/03/05 = Listo). **Carry-over: S43-04 Fix OCR proveedor** — código listo y compila, falta probar con pedimento RAYMOND + Push en repo Control-Tower. Board 18407662134.
